...
5. 우측 상단 "봇 설정"에서 콜백(비동기 응답) 사용을 반드시 켜두세요 — 선택 사항이 아니라 필수 설정입니다. 이 스킬 서버는 항상 콜백으로 실제 답변을 보내는 구조라서, 콜백이 꺼져 있으면 "분석 중이에요..." 안내만 나가고 실제 답변은 영원히 오지 않습니다.
6. 오픈빌더 화면 우측의 "테스트하기" 버튼으로 실제 대화를 시도해, 웹훅이 응답하는지 확인하세요.
7. 문제 없으면 "배포" 메뉴에서 실제 채널에 배포하세요.
8. 이 가이드를 따라 배포한 뒤, 처음 한 번은 실제로 카카오톡에서 질문을 보내서 답변이 정상적으로 오는지 반드시 확인하세요 — 지금까지는 테스트(mock)에서만 확인된 상태이고, 실제 Gemini API 호출로 끝까지 검증된 적은 없습니다.
